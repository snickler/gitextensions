...
namespace GitUI.CommandsDialogs.SettingsDialog.Pages
{
    public partial class FormBrowseRepoSettingsPage : SettingsPageWithHeader
    {
        ...
        {
            AppSettings.ConEmuTerminal.Value = ((IShellDescriptor)cboTerminal.SelectedItem).Type.ToString().ToLowerInvariant();
            base.PageToSettings();
        }
        ...
        protected override void SettingsToPage()
        {
            foreach (var shellType in Enum.GetNames(typeof(ShellType)))
            {
                ...
                if (string.Equals(shellType, AppSettings.ConEmuTerminal.ValueOrDefault, StringComparison.InvariantCultureIgnoreCase))
                {
