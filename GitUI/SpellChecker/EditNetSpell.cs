    ...
    public partial class EditNetSpell : GitModuleControl
    {
        ...
        private void ToggleAutoCompletion()
        {
            ...
            ThreadHelper.JoinableTaskFactory.RunAsync(
                async () =>
                {
                    IEnumerable<AutoCompleteWord>? words = await _autoCompleteListTask.GetValueAsync();
                    if (words is null)
                    {
                        return;
                    }
        ...
        private void UpdateOrShowAutoComplete(bool calledByUser)
        {
            ...
            List<AutoCompleteWord>? list = autoCompleteList?.Where(x => x.Matches(word)).Distinct().ToList();
